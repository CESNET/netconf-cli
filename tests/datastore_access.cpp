...
template <class Exception, typename Callable> void catching(const Callable& what) {
    ...
    if constexpr (std::is_same_v<Exception, void>) {
        what();
    } else if constexpr (std::is_same_v<Exception, std::runtime_error>) {
    ...
    } else if constexpr (std::is_same_v<Exception, DatastoreException>) {
        REQUIRE_THROWS_AS(what(), DatastoreException);
    } else if constexpr (std::is_same_v<Exception, sysrepo::sysrepo_exception>) {
