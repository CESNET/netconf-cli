...
#error "Unknown backend"
    ...
    // FIXME: this hangs on some netopeer callback, where it tries to access /root/.ssh/authorized_keys, but doesn't have permission
    // SECTION("getting /")
    // {
    //     {
    ...
    //     REQUIRE(std::all_of(expected.begin(), expected.end(), [items = datastore.getItems("/")] (const auto& item) { return std::find(items.begin(), items.end(), item) != items.end(); }));
    // }
