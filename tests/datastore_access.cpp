...
#error "Unknown backend"
    ...
    SECTION("moving list instances")
    {
        DatastoreAccess::Tree expected;
        {
            ...
            REQUIRE_CALL(mock, write("/example-schema:players[name='Eve']", std::nullopt, ""s)).TIMES(1);
        ...
        SECTION("begin")
        {
            sourcePath.m_nodes.push_back(dataNode_{module_{"example-schema"}, listElement_{"players", {{"name", "Adam"s}}}});
            REQUIRE_CALL(mock, write("/example-schema:players[name='Adam']", std::nullopt, ""s));
            datastore.moveItem(sourcePath, Absolute::Begin);
            datastore.commitChanges();
            expected = {
        ...
        SECTION("end")
        {
            sourcePath.m_nodes.push_back(dataNode_{module_{"example-schema"}, listElement_{"players", {{"name", "John"s}}}});
        ...
        SECTION("after")
        {
            sourcePath.m_nodes.push_back(dataNode_{module_{"example-schema"}, listElement_{"players", {{"name", "John"s}}}});
            REQUIRE_CALL(mock, write("/example-schema:players[name='John']", ""s, ""s));
            datastore.moveItem(sourcePath, Relative{Relative::Position::After, {{"name", "Eve"s}}});
            datastore.commitChanges();
            expected = {
                {"/example-schema:players[name='Eve']", special_{SpecialValue::List}},
                {"/example-schema:players[name='Eve']/name", "Eve"s},
        ...
        SECTION("before")
        {
            sourcePath.m_nodes.push_back(dataNode_{module_{"example-schema"}, listElement_{"players", {{"name", "John"s}}}});
            REQUIRE_CALL(mock, write("/example-schema:players[name='John']", ""s, ""s));
            datastore.moveItem(sourcePath, Relative{Relative::Position::Before, {{"name", "Adam"s}}});
            datastore.commitChanges();
            expected = {
                {"/example-schema:players[name='Eve']", special_{SpecialValue::List}},
                {"/example-schema:players[name='Eve']/name", "Eve"s},
