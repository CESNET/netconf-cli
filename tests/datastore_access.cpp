...
#error "Unknown backend"
    ...
    // FIXME: this randomly causes other tests to fail, when rerunning the tests, I have no idea why.
    // probably some sort of a problem with asynchronous sysrepo execution...
    ...
    SECTION("moving list instances")
    {
        ...
        // TODO: bug: https://github.com/CESNET/netopeer2/issues/641
        SECTION("after")
        {
            sourcePath = "/example-schema:players[name='John']";
            REQUIRE_CALL(mock, write("/example-schema:players[name='John']", ""s, ""s));
...
class ActionCb: public sysrepo::Callback {
    ...
    {
        auto schema = reinterpret_cast<YangSchema*>(private_data);
...
#error "Unknown backend"
    ...
    srSubscription->rpc_subscribe("/example-schema:noop", rpcCb, nullptr, SR_SUBSCR_CTX_REUSE);
    srSubscription->rpc_subscribe("/example-schema:launch-nukes", rpcCb, nullptr, SR_SUBSCR_CTX_REUSE);
    srSubscription->rpc_subscribe("/example-schema:fire", rpcCb, nullptr, SR_SUBSCR_CTX_REUSE);
    srSubscription->rpc_subscribe("/example-schema:ports/shutdown", actionCb, datastore->schema().get(), SR_SUBSCR_CTX_REUSE);
