    ...
    int module_change(sysrepo::S_Session sess, const char* module_name, sr_notif_event_t event, void*) override
    {
        ...
        while (auto change = sess->get_change_next(it)) {
            auto xpath = change->new_val() ? change->new_val()->xpath() :
                         ...
                         "<no-xpath>";
            auto oldValue = change->old_val() ? boost::optional<std::string>{change->old_val()->val_to_string()} : boost::none;
            auto newValue = change->new_val() ? boost::optional<std::string>{change->new_val()->val_to_string()} : boost::none;
