...
public:
     ...
     *  \return A set of maps, which represent the instances. The map is keyed by the name of the list key. The values in the map, are values of the list keys.
     */
    std::set<std::map<std::string, std::string>> listKeys(const dataPath_& location, const ModuleNodePair& node) const;
