...
int main(int argc, char* argv[])
{
    ...
    auto process = sshProcess(args.at("<hostname>").asString());

    auto datastore = [&] () -> std::shared_ptr<NetconfAccess> {
        try {
            return std::make_shared<NetconfAccess>(process.std_out.native_source(), process.std_in.native_sink());
        } catch (std::runtime_error& ex) {
