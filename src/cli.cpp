...
Options:
  ...
  -e <enable_features>  Features to enable after modules are loaded. Format: <module_name>:<feature>,<module_name>:feature...)";
#else
...
int main(int argc, char* argv[])
{
    ...
    if (const auto& enableFeatures = args["-e"]) {
        ...
        auto it = enableFeatures.asString().begin();
        std::vector<std::pair<std::string, std::string>> parsed;
        auto res = x3::parse(it, enableFeatures.asString().cend(), grammar, parsed);
        if (!res || it != enableFeatures.asString().cend()) {
            std::cerr << "Error parsing feature enable flags: " << enableFeatures.asString() << "\n";
            return 1;
        }
        for (const auto& [module, feature] : parsed) {
            datastore.enableFeature(module, feature);
        }
    }
#else
#error "Unknown CLI backend"
