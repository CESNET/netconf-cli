...
auto getSshOption(ssh_session_struct* session, ssh_options_e option)
{
    char* value;
    ssh_options_get(session, option, &value);
    return std::unique_ptr<char, SshStringDeleter>{value};
}

std::string getHostKey(ssh_session_struct* session)
{
    char* key;
    ssh_session_export_known_hosts_entry(session, &key);
...
bool validateKnownHosts(ssh_session_struct* session)
{
...
ssh_session_struct* createSshSession(const SshOptions& options)
{
    ...
    if (!validateKnownHosts(session)) {
        throw std::runtime_error{"ssh: Host key validation failed."};
    }
