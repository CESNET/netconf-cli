...
struct move_args : x3::parser<move_args> {
    ...
    bool parse(It& begin, It end, Ctx const& ctx, RCtx& rctx, move_& attr) const
    {
        ...
        if (attr.m_move.m_mode == MoveMode::After || attr.m_move.m_mode == MoveMode::Before) {
            ...
            if (attr.m_path.m_nodes.back().m_suffix.type() == typeid(leafListElement_)) {
                ...
                if (attr.m_path.m_nodes.back().m_prefix) {
                    parserContext.m_curModule = attr.m_path.m_nodes.back().m_prefix->m_name;
                }
                parserContext.m_tmpListName = boost::get<listElement_>(attr.m_path.m_nodes.back().m_suffix).m_name;
