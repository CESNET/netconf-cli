...
auto const dataNodesListEnd_def =
        initializePath >> dataNode % '/' >> '/' >> dataNodeList |
        initializePath >> x3::attr(decltype(dataPath_::m_nodes)()) >> dataNodeList;

auto const dataPathListEnd_def =
        initializePath >> absoluteStart >> x3::attr(decltype(dataPath_::m_nodes)()) >> x3::eoi |
        initializePath >> -(absoluteStart) >> dataNodesListEnd;

auto const schemaPath_def =
        initializePath >> absoluteStart >> x3::attr(decltype(schemaPath_::m_nodes)()) >> x3::eoi |
        initializePath >> -(absoluteStart) >> schemaNode % '/';
