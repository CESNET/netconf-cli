...
struct move_absolute_table : x3::symbols<move_::Absolute> {
    move_absolute_table()
    {
        add
            ("begin", move_::Absolute::Begin)
            ("end", move_::Absolute::End);
    }
} const move_absolute_table;

struct move_relative_table : x3::symbols<move_::Relative::Position> {
    move_relative_table()
    {
        add
            ("before", move_::Relative::Position::Before)
            ("after", move_::Relative::Position::After);
    }
...
struct move_args : x3::parser<move_args> {
    ...
    bool parse(It& begin, It end, Ctx const& ctx, RCtx& rctx, move_& attr) const
    {
        ...
        attr.m_destination = move_::Relative{};
        res = (space_separator >> move_relative_table).parse(begin, end, ctx, rctx, std::get<move_::Relative>(attr.m_destination).m_position);
        ...
        if (std::holds_alternative<leafListElement_>(attr.m_source.m_nodes.back().m_suffix)) {
            ...
            if (res) {
                std::get<move_::Relative>(attr.m_destination).m_path = value;
            }
        } else {
            ...
            if (res) {
                std::get<move_::Relative>(attr.m_destination).m_path = listInstance;
            }
        }
