...
auto const dataPath_def =
        ...
        initializePath >> -(absoluteStart) >> dataNode % '/' >> (trailingSlash >> createSuggestions | (&space_separator | x3::eoi));
...
auto const dataPathListEnd_def =
        initializePath >> absoluteStart >> x3::attr(decltype(dataPath_::m_nodes)()) >> x3::attr(TrailingSlash::NonPresent) >> x3::eoi |
        initializePath >> -(absoluteStart) >> dataNodesListEnd >> (trailingSlash >> createSuggestions | (&space_separator | x3::eoi));
...
auto const schemaPath_def =
        ...
        initializePath >> -(absoluteStart) >> schemaNode % '/' >> (trailingSlash >> createSuggestions | (&space_separator | x3::eoi));
