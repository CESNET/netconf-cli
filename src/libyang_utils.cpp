...
void lyNodesToTree(DatastoreAccess::Tree& res, const std::vector<libyang::DataNode> items, std::optional<std::string> ignoredXPathPrefix)
{
...
DatastoreAccess::Tree rpcOutputToTree(const std::string& rpcPath, libyang::DataNode output)
{
    ...
    lyNodesToTree(res, {*outputNode}, joinPaths(std::string{outputNode->path()}, "/"));
    return res;
}
