...
void Interpreter::operator()(const set_& set) const
{
    ...
    m_datastore.setLeaf(pathToDataString(set.m_path), set.m_data);
}
...
void Interpreter::operator()(const create_& create) const
{
    ...
    m_datastore.createPresenceContainer(pathToDataString(create.m_path));
}
...
void Interpreter::operator()(const delete_& delet) const
{
    ...
    m_datastore.deletePresenceContainer(pathToDataString(delet.m_path));
}
