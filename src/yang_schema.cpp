...
bool YangSchema::isContainer(const path_& location, const ModuleNodePair& node) const
{
    ...
    const auto schemaSet = std::shared_ptr<std::vector<std::shared_ptr<Schema_Node>>>(set->schema());
    if (!set)
        return false;
    return set->number() == 1 && (*(schemaSet->begin()))->nodetype() == LYS_CONTAINER;
}
...
bool YangSchema::isLeaf(const path_& location, const ModuleNodePair& node) const
{
    ...
    if (!set)
        return false;
    return set->number() == 1 && (*(schemaSet->begin()))->nodetype() == LYS_LEAF;
}
...
bool YangSchema::isList(const path_& location, const ModuleNodePair& node) const
{
    ...
    if (!set)
        return false;
    return set->number() == 1 && (*(schemaSet->begin()))->nodetype() == LYS_LIST;
}
...
bool YangSchema::isPresenceContainer(const path_& location, const ModuleNodePair& node) const
{
    ...
    const auto set = getNodeSet(location, node);
    const auto schemaSet = std::shared_ptr<std::vector<std::shared_ptr<Schema_Node>>>(set->schema());
    const auto container = std::make_shared<Schema_Node_Container>(schemaSet->back());
...
bool YangSchema::leafEnumHasValue(const path_& location, const ModuleNodePair& node, const std::string& value) const
{
    ...
    const auto set = getNodeSet(location, node);
    const auto schemaSet = std::shared_ptr<std::vector<std::shared_ptr<Schema_Node>>>(set->schema());
    const auto leaf = std::make_shared<Schema_Node_Leaf>(schemaSet->back());
    const auto enm = std::shared_ptr<std::vector<S_Type_Enum>>(leaf->type()->info()->enums()->enm());
...
const std::set<std::string> YangSchema::listKeys(const path_& location, const ModuleNodePair& node) const
{
    ...
    const auto set = getNodeSet(location, node);
    const auto schemaSet = std::shared_ptr<std::vector<std::shared_ptr<Schema_Node>>>(set->schema());
    const auto list = std::make_shared<Schema_Node_List>(schemaSet->back());
    const auto keysVec = std::shared_ptr<std::vector<S_Schema_Node_Leaf>>(list->keys());
    std::transform(keysVec->begin(), keysVec->end(), std::inserter(keys, keys.begin()),
            [] (const auto& it) {return it->name();});
    for (const auto it : *keysVec) {
        keys.insert(it->name());
    }
    return keys;
}
...
{
    const auto modules = std::shared_ptr<std::vector<S_Module>>(m_context->get_module_iter());
...
std::set<std::string> YangSchema::childNodes(const path_& path) const
{
    ...
    if (path.m_nodes.empty()) {
        const auto nodeVec = std::shared_ptr<std::vector<S_Schema_Node>>(m_context->data_instantiables(0));
