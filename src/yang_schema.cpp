...
yang::TypeInfo YangSchema::impl_leafType(const NodeType& node) const
{
    ...
    resolveType = [&resolveType, leaf, leafUnits](const libyang::types::Type& type) -> yang::TypeInfo {
        ...
        case libyang::LeafBaseType::InstanceIdentifier: {
            resType.emplace<yang::InstanceIdentifier>(type.asInstanceIdentifier().requireInstance() ?
                    yang::InstanceIdentifier::RequireInstance::Required :
                    yang::InstanceIdentifier::RequireInstance::NotRequired);
            break;
        }
        default:
            using namespace std::string_literals;
