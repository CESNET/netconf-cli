...
void Schema::addContainer(const std::string& location, const std::string& name, bool isPresence)
{
...
{
    std::string locationString = pathToString(location);
    assert(isContainer(location, name));
    return boost::get<yang::container>(children(locationString).at(name)).m_presence;
}
