...
const std::set<std::string> StaticSchema::validIdentities(const schemaPath_& location, const ModuleNodePair& node, const Prefixes prefixes) const
{
    ...
    std::set<ModuleValuePair> validIdentities;
    getIdentSet(leaf.m_identBase, validIdentities);

    std::set<std::string> res;
    std::transform(validIdentities.begin(), validIdentities.end(), std::inserter(res, res.end()), [location, node, prefixes] (const auto& it) {
