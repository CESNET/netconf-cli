...
void StaticSchema::addLeafUnion(const std::string& location, const std::string& name, std::vector<yang::leaf::UnionInfo> types)
{
    yang::leaf toAdd;
...
yang::LeafDataTypes StaticSchema::leafrefBaseType(const std::string& path)
{
...
std::vector<yang::LeafDataTypes> StaticSchema::unionTypes(const schemaPath_& location, const ModuleNodePair& node) const
{
    ...
    std::function<void(const std::vector<yang::leaf::UnionInfo>&)> recurseUnion = [this, &res, &recurseUnion] (const std::vector<yang::leaf::UnionInfo>& unionInfo) {
        for (auto info : unionInfo) {
            if (info.m_type == yang::LeafDataTypes::LeafRef) {
                res.push_back(leafrefBaseType(info.m_leafRefSource));
                continue;
            }

            if (info.m_type == yang::LeafDataTypes::Union) {
                recurseUnion(info.m_unionTypes);
                continue;
            }
    ...
    recurseUnion(leaf.m_unionTypes);
