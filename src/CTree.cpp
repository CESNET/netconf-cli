...
{
    public:
        InvalidNodeException(const std::string& node_name) : std::invalid_argument(node_name)
        {
...
{
    if (node == "..") return true;
    const std::unordered_set< std::string >& children = getChildren(m_cur_dir); //first, get a reference to all children
