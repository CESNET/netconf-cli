...
public:
    ...
    // Adds a new module passed as a YANG string.
    void addSchemaString(const char* schema);

    // Adds a new module from a file.
    void addSchemaFile(const char* filename);

    // Adds a new directory for schema lookup.
...
private:
    ...
    // returns a set of nodes, that match the location and name criteria.
    std::shared_ptr<Set> getNodeSet(const path_& location, const ModuleNodePair& node) const;

    // returns a single Schema_Node if the criteria matches only one, otherwise nullptr.
