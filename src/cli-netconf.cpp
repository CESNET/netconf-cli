...
std::string getHostKeyFingerPrint(const SshSessionUniquePtr& session)
{
    ssh_key_struct* key;
    ...
    ssh_clean_pubkey_hash(&keyHash); // I would love to use a unique_ptr wrapper I use everywhere, but libssh needs a double pointer here for some reason...

    return std::string{keyHashStrPtr.get()};
}
...
bool verifyHost(const SshSessionUniquePtr& session)
{
    ...
    case SSH_KNOWN_HOSTS_UNKNOWN: // Known hosts file exists, but the host is not present
    {
        ...
        std::cout << "ECDSA key fingerprint is " << getHostKeyFingerPrint(session) << "." << std::endl;
    ...
    case SSH_KNOWN_HOSTS_CHANGED:
        ...
        std::cout << getHostKeyFingerPrint(session) << std::endl;
