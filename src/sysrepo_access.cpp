...
void SysrepoAccess::reportErrors()
{
    auto errors = m_session->get_last_errors();
    std::string msg = "Error from sysrepo:\n";
    for (size_t i = 0; i < errors->error_cnt(); i++) {
        auto error = errors->error(i);
        msg += "\tMessage: ";
        msg += error->message() ? error->message() : "<unspecifed>";
        msg += "\n";
