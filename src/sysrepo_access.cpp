...
DatastoreAccess::Tree SysrepoAccess::getItems(const std::string& path) const
{
    ...
    try {
        ...
        if (config) {
            lyNodesToTree(res, config->tree().siblings());
        }
...
DatastoreAccess::Tree SysrepoAccess::execute(const std::string& path, const Tree& input)
{
    ...
    return rpcOutputToTree(output.tree());
}
...
std::vector<ListInstance> SysrepoAccess::listInstances(const std::string& path)
{
    ...
    auto instances = lists->tree().findXPath(path.c_str());
    if (instances.empty()) {
        return res;
    }
...
std::string SysrepoAccess::dump(const DataFormat format) const
{
    ...
    auto str = root->tree().printStr(format == DataFormat::Xml ? libyang::DataFormat::XML : libyang::DataFormat::JSON, libyang::PrintFlags::WithSiblings);
    if (!str) {
        return "";
    }
