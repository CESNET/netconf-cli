...
public:
    ...
    std::shared_ptr<libyang::Data_Node> getConfig(const NC_DATASTORE datastore, const std::string& filter); // TODO: arguments...
    std::shared_ptr<libyang::Data_Node> get(const std::string& filter);
    std::string getSchema(const char* identifier, const char* version);
