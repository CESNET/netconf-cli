...
std::string Session::getSchema(const std::string& identifier, const std::string& version)
{
    NETCONF_CLIENT_TRACE_FUNC;
    auto rpc = impl::guarded(nc_rpc_getschema(identifier.c_str(), version.c_str(), nullptr, NC_PARAMTYPE_CONST));
    if (!rpc) {
