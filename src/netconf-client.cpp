...
std::optional<libyang::DataNode> do_rpc(client::Session* session, managed_rpc&& rpc)
{
    uint64_t msgid;
    NC_MSG_TYPE msgtype;
    ...
    while (true) {
        ...
        if (envp) {
            libyang::wrapRawNode(envp); // Just to free the thing
        }
        ...
        case NC_MSG_NOTIF:
            continue;
        default:
            if (!raw_reply) { // <ok> reply, or empty data node
                return std::nullopt;
            }

            return libyang::wrapRawNode(raw_reply);
        }
    }
    __builtin_unreachable();
}

client::ReportedError make_error() // FIXME: figure this out.
{
...
std::optional<libyang::DataNode> Session::get(const std::optional<std::string>& filter)
{
    ...
    return impl::do_rpc(this, std::move(rpc));
}
...
std::string_view Session::getSchema(const std::string_view identifier, const std::optional<std::string_view> version)
{
    ...
    auto reply = impl::do_rpc(this, std::move(rpc));
