...
std::string impl_dumpConfig(const std::vector<std::shared_ptr<libyang::Data_Node>>& nodes, LYD_FORMAT format)
{
    std::stringstream ss;
    std::transform(nodes.begin(), nodes.end(), std::experimental::make_ostream_joiner(ss, "\n"), [format] (const auto& tree) { return tree->print_mem(format, 0); });
    return ss.str();
}
