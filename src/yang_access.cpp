...
{
    // Using libyang::get_ly_errors requires wrapping m_ctx in libyang::Context
...
void YangAccess::impl_removeNode(const std::string& path)
{
    ...
    if (!set) { // Error
        ...
        // Otherwise just return, the datastore is empty and lyd_find_path doesn't like nullptr as a first argument
        return;
    }
    if (set->number == 0) {
        return;
