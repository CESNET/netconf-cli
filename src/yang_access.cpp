...
void impl_unlink(DatastoreType& datastore, libyang::DataNode what)
{
    ...
    std::cerr << "datastore->path() = " << datastore->path() << "\n";
    if (datastore == what) {
...
struct impl_moveItem {
    ...
    void operator()(yang::move::Absolute absolute) const
    {
        ...
        if (set.front() == set.back()) { // m_sourceNode is the sole instance, do nothing
            return;
        }
...
private:
    void doUnlink() const
    {
        // This method is unneeded, the insert methods unlink by themselves.
...
// FIXME: the return type should probably be std::optional<std::string>
std::string YangAccess::dump(const DataFormat format) const
{
    ...
    auto str = m_datastore->printStr(format == DataFormat::Xml ? libyang::DataFormat::XML : libyang::DataFormat::JSON, libyang::PrintFlags::WithSiblings);
    if (!str) {
        return "";
    }
...
void YangAccess::addDataFile(const std::string& path)
{
    ...
    auto dataNode = m_ctx.parseDataPath(path.c_str(), firstChar == '{' ? libyang::DataFormat::JSON : libyang::DataFormat::XML);
