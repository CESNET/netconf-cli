...
auto fillMap(DatastoreAccess::Tree& res, const std::vector<std::shared_ptr<libyang::Data_Node>> items, std::optional<std::string> ignoredXPathPrefix = std::nullopt) {
...
DatastoreAccess::Tree NetconfAccess::executeRpc(const std::string& path, const Tree& input)
{
    ...
    for (const auto& [k, v] : input) {
        auto node = m_schema->dataNodeFromPath(path + "/" + k, leafDataToString(v));
        root->merge(node, 0);
    }
