...
void NetconfAccess::setLeaf(const std::string& path, leaf_data_ value)
{
    ...
    doEditFromDataNode(*nodes.newParent);
}
...
void NetconfAccess::deleteItem(const std::string& path)
{
    ...
    if (nodes.newNode->isOpaque()) {
        nodes.newNode->newAttrOpaqueJSON("ietf-netconf", "ietf-netconf:operation", "delete");
    } else {
        nodes.newNode->newMeta(*m_schema->getYangModule("ietf-netconf"), "operation", "delete");
    }
    doEditFromDataNode(*nodes.newParent);
}
...
void NetconfAccess::moveItem(const std::string& source, std::variant<yang::move::Absolute, yang::move::Relative> move)
{
    ...
    auto sourceNode = *(nodes.newNode->findPath(source.c_str()));
    ...
    if (std::holds_alternative<yang::move::Relative>(move)) {
        ...
        if (m_schema->nodeType(source) == yang::NodeTypes::LeafList) {
            ...
            sourceNode.newMeta(yangModule, "key", instanceToString(relative.m_path, std::string{nodes.newNode->schema().module().name()}).c_str());
        }
    }
...
std::vector<ListInstance> NetconfAccess::listInstances(const std::string& path)
{
    ...
    for (const auto& keyLeaf : keys) {
        ...
        // here.
        nodes.newNode->newPath(keyLeaf.name().data(), nullptr, libyang::CreationOptions::Opaque);
    }
    ...
    auto instances = m_session->get(std::string{*nodes.newParent->printStr(libyang::DataFormat::XML, libyang::PrintFlags::WithSiblings)});
