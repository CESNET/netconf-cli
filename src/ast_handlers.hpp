...
struct createValueSuggestions_class {
    ...
    void on_success(Iterator const& begin, Iterator const&, T&, Context const& context)
    {
        ...
        std::copy_if(listInstances.begin(), listInstances.end(), std::inserter(filteredInstances, filteredInstances.end()), [&parserContext] (const auto& instance) {
            // For every instance we got from listKeys, iterate over the partial
            // instance we already have and check if our instance fits into the one
            // we got from listKeys.
            for (auto [k, v] : parserContext.m_tmpListKeys) {
                if (instance.at(k) != v) {
                    return false;
                }
            }
